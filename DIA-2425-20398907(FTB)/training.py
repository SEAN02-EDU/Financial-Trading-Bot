...
# --------------------------------------------------
ta_file_path = r"C:\Users\User\Downloads\DIA-2425-20398907(FTB)\TSLA_final.csv"
sa_file_path = None  # or another sentiment file if needed
include_sa = False  # Set to False if you do not want to include sentiment analysis data

start_date = pd.to_datetime("2021-10-01")
end_date   = pd.to_datetime("2022-09-29")

# Load Technical Analysis / Financial dataset.
df_ta = pd.read_csv(ta_file_path)
...
# Optionally load and merge a sentiment dataset.
if include_sa:
    df_sa = pd.read_csv(sa_file_path)
    if "Unnamed: 0" in df_sa.columns:
        df_sa.drop("Unnamed: 0", axis=1, inplace=True)
    if "Date" not in df_sa.columns:
        raise ValueError("SA dataset must include a 'Date' column.")
    df_sa["Date"] = pd.to_datetime(df_sa["Date"])
    df_sa = df_sa.sort_values("Date").reset_index(drop=True)
    print("SA dataset shape:", df_sa.shape)
    
    df_merged = pd.merge(df_ta, df_sa, on="Date", how="inner")
...
selected_features = ['Close', 'ADX', 'MACD', 'RSI', 'OBV', 'BB_upper', 'STOCH_D', 'ROE', 'EPS', 'sentiment_score']
...
# --------------------------------------------------
callbacks = [
    EarlyStopping(monitor='val_loss', min_delta=1e-10, patience=10, verbose=1),
...
for i in range(7):  # forecast horizon of 7 days
...
class TradingEnvWithForecast(gym.Env):
    ...
    def __init__(self, df, forecasted_prices, dates, seq_length=1, initial_balance=10000):
        ...
        self.current_step = seq_length  # Start when we have at least seq_length days.
